...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = np.exp(X @ W) / np.sum(np.exp(X @ W), axis = 1).reshape(-1,1)
	NLL = np.sum( np.sum( y * np.log(mu) , axis = 1 ), axis = 0) + reg * np.trace(X @ W) /2
	"*** END YOUR CODE HERE ***"
	return NLL
...
def predict(X, W):
	...
	"*** YOUR CODE HERE ***"
	mu = np.exp(X @ W) / np.sum(np.exp(X @ W), axis = 1).reshape(-1,1)
	y_pred = np.argmax(mu, axis = 1 ).reshape(-1,1)
	"*** END YOUR CODE HERE ***"
	return y_pred
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		W_opt, list_nll = grad_descent(X_train, y_train_OH, reg = reg, lr=2e-5)
