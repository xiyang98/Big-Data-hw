...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X @ W)
	nll = - (y * np.log(mu) + (1-y) * np.log(1-mu)).sum() + (reg * np.linalg.norm(W)**2)/2
	"*** END YOUR CODE HERE ***"
	return nll



def grad_descent(X, y, reg=0.0, lr=2e-4, eps=1e-6, max_iter=500, print_freq=20):
...
def newton_step(X, y, W, reg=0.0):
	...
	grad = grad_logreg(X, y, W, reg)
	mu = sigmoid(X @ W)
	dmatrix = np.diag(np.squeeze(mu*(1-mu)))
	Hd = X.T @ dmatrix @ X + reg * np.eye(X.shape[1])
...
def get_description(X, y, W):
	...
	precision = both1/predlabel1
	recall = both1/truelabel1
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	for i in range(len(reg_list)):
		W, list_nll = grad_descent(X_train, y_train, reg_list[i], lr=2e-4)
		a_list.append(get_description(X_test, y_test, W)[0])
		p_list.append(get_description(X_test, y_test, W)[1])
		r_list.append(get_description(X_test, y_test, W)[2])
		f1_list.append(get_description(X_test, y_test, W)[3])
	"*** END YOUR CODE HERE ***"
	...
	"*** YOUR CODE HERE ***"
	maxaccuracy = a_list[0]
	maxindex = 0
	for i in range(len(reg_list)):
		if maxaccuracy < reg_list[i]:
			maxaccuracy = reg_list[i]
			maxindex = i
	reg_opt = reg_list[maxindex]
	"*** END YOUR CODE HERE ***"
	return reg_opt
