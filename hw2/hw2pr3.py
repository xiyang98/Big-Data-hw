...
def find_RMSE(W, X, y):
	"""	This function takes in three arguments:
		...
		This function calculates and returns the b, RMSE
	"""
	# TODO: Solve for the root mean-squared error, RMSE
	"*** YOUR CODE HERE ***"
	y_pred = predict(W,X)
	y_actual = y
	MSE = (np.linalg.norm(y_actual - y_pred))**2 / X.shape[0]
	RMSE = np.sqrt(MSE)
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	"""
	# Generate a list of RMSE, RMSE_list
	RMSE_list = []
	reg_list = []
	W_list = []
	"*** YOUR CODE HERE ***"
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	n = X.shape[0]
	A_mod = X.T @ (np.eye(n) - np.ones((n,n))/n)
	W_opt = np.linalg.solve(A_mod @ X + reg * np.eye(A_mod.shape[0]),A_mod @ y,)
	b_opt = (y - X @ W_opt).sum() / n
	"*** END YOUR CODE HERE ***"
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		MSE = (np.linalg.norm(y_train - (X_train@W + b))**2) / m_train
		RMSE = np.sqrt(MSE)
		train_rmse = RMSE

		MSE = (np.linalg.norm(y_val - (X_val@W + b))**2) / m_val
		RMSE = np.sqrt(MSE)
		val_rmse = RMSE
		...
		W_grad = (X_train.T @ X_train @ W + reg * np.eye(n) @ W + X_train.T @ (b - y_train)) / m_train
		b_grad = ((X_train @ W).sum() - y_train.sum() + b * m_train) / m_train
