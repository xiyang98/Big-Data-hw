...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for ind in range(clusters.shape[0]):
			if len(X[label==ind]) != 0:
				clusters[ind] = X[label==ind].mean(axis=0)
		"*** END YOUR CODE HERE ***"
...
def k_means_cost(X, clusters, label):
	...
	"*** YOUR CODE HERE ***"
	mu = clusters[label.flatten()]
	cost = (np.linalg.norm(X - mu, axis = 1)**2) . sum()
	"*** END YOUR CODE HERE ***"
	return cost
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	clusters = k_means(X, opt_k)[0]
	data_plot = plt.scatter(X[:, 0], X[:, 1])
	center_plot= plt.scatter(clusters[:, 0], clusters[:, 1], c='r', s=100 )
